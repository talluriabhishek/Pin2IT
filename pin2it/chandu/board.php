...
$sql_prep_boards ->store_result();
$count1 = $sql_prep_boards->num_rows;
$sql_prep_boards->bind_result($boardname1);
...
$conn2 = getMeDB();
$sql_pins = "select picId  from pins where username = ? and boardname = ?";
$sql_prep_pins = $conn2->prepare($sql_pins);
$sql_prep_pins->bind_param("ss", $username,$boardname);
$sql_prep_pins->execute();
$sql_prep_pins ->store_result();
$sql_prep_pins->bind_result($picId);
?>
			...
			<ul class="nav navbar-nav pull-right">
				...
				<ul class="dropdown-menu">
						...
						<li>
							...
							<li> 
							<?php
								echo '<a href="inbox.html?friend='. $username6.'">
												<span class="subject">
			...
			<ul class="page-sidebar-menu" data-keep-expanded="false" data-auto-scroll="true" data-slide-speed="200" style="margin-top:25px;">
				<li class="start ">
					...
					<?php
						...
						<?php
							if(isset($_GET['otheruser'])){
								echo '<a href="board.php?boardname='.$boardname.'&otheruser='.$username.'"> '.$boardname.'</a>';
							}else{
	...
	<div class="page-content-wrapper">
		<div class="page-content">
			...
			if(isset($_GET['otheruser'])){
				...
				$boardname5 = $_GET['boardname'];
				
				echo '
				<input name="hiddenBoardId1" type="hidden" value= "'.$_GET['boardname'].'"/>
				<input name="otherUser1" type="hidden" value="'.$otheruser5.'"/> ';
				
				
				include("dbconnect.php");
				$sql3="SELECT count(*) as thisFollow from followstreams where username = '$username5' and username2 = '$otheruser5' and boardname2 = '$boardname5'";
				$query4 = mysql_query($sql3,$link) or die(mysql_error());
				$result4=mysql_fetch_array($query4);
			?>
			<form action="followstream.php" class="form-horizontal form-row-sepe" method = "post" enctype = "multipart/form-data">
				<div class="form-actions">
				<?php
					echo '<input type="hidden" name="boardname" value = '.$boardname.'>';
					echo '<input type="hidden" name="otheruser" value = '.$otheruser5.'>';
				?>
				<button class="btn btn-primary col-md-2 pull-right followbutton" id="followButton" <?php if($result4["thisFollow"] != 0){ echo 'disabled';}?> >Follow Board</button><br>
				</div>
			</form>
			...
			<div class="row">
				...
				<input id="hiddenBoardId" type="hidden" value="<?php echo $_GET['boardname']; ?>"/>
				<input id="otherUser" type="hidden" value="<?php echo $_GET['otheruser']; ?>"/>
				<div class="col-md-12">
				
					<div class="row mix-grid">
						<?php
							$count2 = $sql_prep_pins->num_rows;
							if($count2 == 0){
								?>
								<div>
									<h3 class=" page-title ">
									There are no images in this board.
								</h3>
								</div>
							<?php	
							}
							...
							<div class="col-md-3 col-sm-4 mix category_1">
								<div class="mix-inner">
									...
									<div id="commentslist<?php echo $picId; ?>">
										...
										<input class="commentfield" id="commentfield<?php echo $picId; ?>" placeholder="Enter the comment"></input>
									</div>
								</div>
