...
$sql_prep_streams = $conn2->prepare($sql_streams);
$sql_prep_streams->bind_param("s", $username);
$sql_prep_streams->execute();
$sql_prep_streams ->store_result();

$sql_prep_streams->bind_result($streamname2);
$count3 = $sql_prep_streams->num_rows;

$conn3 = getMeDB();
$sql_follow_pics = "select p.picId,p.boardname from followstreams as f,pins as p where f.username = ? and f.streamname = ? and f.username2 = p.username and f.boardname2 = p.boardname";
$sql_prep_follow_pics = $conn3->prepare($sql_follow_pics);
$sql_prep_follow_pics->bind_param("ss", $username,$streamname);
$sql_prep_follow_pics->execute();
$sql_prep_follow_pics ->store_result();
$sql_prep_follow_pics->bind_result($picId,$boardname2);
$conn4 = getMeDB();
...
<head>
<meta charset="utf-8"/>
<title>Metronic | Page Layouts - Stream</title>
			...
			<ul class="page-sidebar-menu" data-keep-expanded="false" data-auto-scroll="true" data-slide-speed="200" style="margin-top:25px;">
				...
				<li class="active open">
					...
					<?php
						...
						<li> 
						<?php
							echo '<a href="stream.php?streamname='.$streamname2.'"> '.$streamname2.'</a>';
						?>
						</li>
					<?php }?>
			...
			<div class="row">
				...
				<div class="col-md-12">
					<div class="row mix-grid">
							...
							$tempboard = '';
							$conn4 = getMeDB();
							$sql_streams4 = "select streamname from streams where username = ?";
							$sql_prep_streams4 = $conn4->prepare($sql_streams4);
							$sql_prep_streams4->bind_param("s", $username);
							$sql_prep_streams4->execute();
							$sql_prep_streams4->store_result();
							$sql_prep_streams4->bind_result($streamname4);
							$count4 = $sql_prep_streams4->num_rows;
							if($count4 == 0){
							?>
								<div>
									<h3 class="errormesg ">
									<?php echo 'There are no streams. Create One !!'; ?>
									</h3>
								</div>
							<?php
							}else{
							?>
								<div>
									<h3 class="boardnameonbar ">
									<?php echo 'Stream : '.$streamname ; ?>
									</h3>
								</div>
							
							<?php							
							}
							while($sql_prep_follow_pics->fetch()) 
							{
